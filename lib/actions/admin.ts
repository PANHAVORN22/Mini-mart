...
export async function updateBeerStock(beerId: string, stock: number, price: number) {
  const supabase = await getSupabaseServerClient()

  const { error } = await supabase.from("beers").update({ stock, price }).eq("id", beerId)
...
export async function toggleUserPremium(userId: string, isPremium: boolean) {
  ...
  const premiumExpiresAt = isPremium ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString() : null
